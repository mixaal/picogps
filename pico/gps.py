...
class GpsHandler(object):
    def __init__(self, uart, baudrate, fs):
        self.fs = fs
        self.gnss_l76b = L76X(uartx=uart,_baudrate=baudrate)
    ...
    def log_gps(self):
        ...
        # if gnss available,then print the nmea0183 sentence
            ...
            #print(sentence)
                if sentence:
                    gps_record = {
                        'lat': self.parser.latitude[0],
                        'lon': self.parser.longitude[0],
                        'hdir': self.parser.latitude[1],
                        'vdir': self.parser.longitude[1],
                        'alt': self.parser.altitude,
                        'fix': self.parser.fix_stat,
                        'sats': self.parser.satellites_in_use,
                        'timestamp': time_ms()
                    }
                    records.append(gps_record)
                    if len(records) > 30:
                        self.fs.save_data(records)
                        records = []
